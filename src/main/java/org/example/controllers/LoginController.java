...
import org.example.utils.NotificationUtil;
...
public class LoginController {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                if (BCrypt.checkpw(password, hash)) {
                    ...
                    // ── BAN CHECK ──────────────────────────────────────────
                    org.example.Services.ServiceBan serviceBan = new org.example.Services.ServiceBan();
                    org.example.entities.Ban activeBan = serviceBan.getActiveBan(userId);
                    ...
                    if (role.equalsIgnoreCase("Admin") ||
                        ...
                        targetFxml  = "/org/example/fxml/dashboard.fxml";
                        targetTitle = "LearnFlex+ Dashboard";
                    } else {
                        targetFxml  = "/org/example/fxml/front.fxml";
        ...
        new Thread(() -> {
            try {
                com.google.api.services.oauth2.model.Userinfo userInfo =
                        org.example.Services.GoogleAuthService.authenticate();
                ...
                javafx.application.Platform.runLater(() -> {
                    try {
                        org.example.Services.ServiceUsers serviceUsers = new org.example.Services.ServiceUsers();
                        org.example.entities.Users found = serviceUsers.findByEmail(email);

                        if (found == null) {
                            org.example.entities.Users newUser = new org.example.entities.Users();
                        ...
                        final org.example.entities.Users finalUser = found;
                        ...
                        // Ban check
                        org.example.Services.ServiceBan serviceBan = new org.example.Services.ServiceBan();
                        org.example.entities.Ban activeBan = serviceBan.getActiveBan(finalUser.getId());
                        if (activeBan != null) {
                            showNotification("Compte suspendu : " + activeBan.getReason(), false);
                            return;
                        }
                        ...
                        String targetFxml = (role.equalsIgnoreCase("Admin") ||
                                role.equalsIgnoreCase("Enseignant"))
                                ? "/org/example/fxml/dashboard.fxml" : "/org/example/fxml/front.fxml";
    ...
    public void goToSignup(MouseEvent event) {
        switchScene(event, "/org/example/fxml/Signup.fxml", "Signup");
    }
    ...
    private void openFaceAuth(ActionEvent event) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/org/example/fxml/face_auth.fxml"));
    ...
    private void goToForgotPassword(MouseEvent event) {
        switchScene(event, "/org/example/fxml/forgot_password.fxml", "Forgot Password");
    }

    @FXML
    private void goToFront(MouseEvent event) {
        switchScene(event, "/org/example/fxml/front.fxml", "LearnFlex+");
    }
