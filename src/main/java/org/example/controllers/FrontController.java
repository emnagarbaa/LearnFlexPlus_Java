    ...
    private void goToQuestionnaire() {
        try {
            FXMLLoader loader = new FXMLLoader(
                    getClass().getResource("/org/example/fxml/quiz_front.fxml")
            );

            Parent root = loader.load();

            // si tu es dans une fenêtre normale
            Stage stage = (Stage) logo.getScene().getWindow();            stage.setScene(new Scene(root));
            stage.setTitle("Quiz - LearnFlex+");
